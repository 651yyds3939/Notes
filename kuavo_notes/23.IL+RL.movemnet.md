...
> **📌 23.x 扩展版已发布**：`23.1`–`23.6` 各约 1000 行。**终端命令以 `23.6` 扩展版为准**（lite URDF、2048 envs、逐步解读）。
...
**RL 切换逻辑速查**（详 **15.3** / **15.4** / **23.4** §9.5）：

| 场景 | 操作 | 连招 |
| --- | --- | --- |
| **MuJoCo Sim2Sim** | `joystickSimulator.py` **独立窗口**键盘 | **R → F → P** |
| **真机 H12** | 物理遥控器 | **F 拨杆** WBC 起立 → **B 键** RL |
...
- [S49 四代机 IL+RL 全身舞全流程终端指令](#s49-四代机-ilrl-全身舞全流程终端指令)
  - [目录](#目录)
  - [第零章：一次性准备](#第零章一次性准备)
  - [第一章：训练前护城河（每次训练前执行）](#第一章训练前护城河每次训练前执行)
  - [第二章：Isaac Lab 训练（S49 全身舞）](#第二章isaac-lab-训练s49-全身舞)
...
### 2.2 录像抽检（内存小勿开大 num_envs；详 **23.6** §1.4）
...
### 3.3 录像查看 play 效果（可选；详 **23.6** §1.4.2）
...
### 5.2 Sim2Sim 键盘解锁连招（与 **15.3** §5.2 一致）

> **仿真 ≠ 真机**：Sim2Sim 用 **`joystickSimulator.py` 窗口的键盘**；真机用 **H12 遥控器**（见第六章 §6.4）。

等 MuJoCo **Run** 变绿、WBC 微蹲—起立站稳 **≥2 秒** 后：

1. 确认 launch 已弹出 **`joystickSimulator` 独立终端**（`joystick_type:=sim`；**不是** roslaunch 那个窗口）。
2. 在 **joystickSimulator 窗口**内依次短按（经 `/joy` 话题，与 **15.3** 同源）：
   - **`R`** → `BUTTON_RL`：激活 RL，加载 `policy_s49.onnx`
   - **`F`** → `BUTTON_START`：进入准备态（`real_initial_start`）
   - **`P`** → `BUTTON_LB`：`Walkenable=true`，解除安全锁
3. 舞蹈策略速度指令恒为 **0**，**不需要** W/A/S/D。

若窗口未弹出 / 日志 `joystickSimulator process has died`（Docker 内 `gnome-terminal` 失败），容器内手动：
...
### 6.4 真机 H12 遥控器 SOP（与 **15.4** §5.2 一致）

> **真机不用键盘 R/F/P**；Sim2Sim 才用 **joystickSimulator 的 R→F→P**。

**点火前（15.4 §5.2）**

- **E 拨杆** → 最左（节点挂起 / 急停保护）
- **F 拨杆** → 最右（悬挂准备姿态）
- 双人 SOP：助手守 **E 拨杆** + `Ctrl+C`；主操站侧后方 45° 扶躯干

**接管三步**

1. **F 拨杆向左拨到底** → 经典 WBC 平稳起立（S49 `com_z ≈ 0.85`），等 **≥2 秒**、无啸叫
2. **松开扶躯干的手**
3. **点按 B 键** → INTO RL-Control（`BUTTON_RL`），舞蹈策略接管
