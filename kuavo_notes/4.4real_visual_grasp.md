...
---
![alt text](31390c905e51b4cc97960c778a975114.png)
# 第零章：“无痛抄作业”保姆级实机运行教程
...
* **DIY：** 重写了 `auto_grasp.py`。我们引入了“10帧中值滤波”、“防万向节死锁的旋转矩阵右乘”、“TCP物理齿轮偏心补偿”（精确到毫米级的 `TCP_OFFSET_X/Y`），以及极其复杂的“倒推法规划”和“3D L型外扩避障通道”。
...
我们的系统被严格划分为上位机（算力大脑）与下位机（运动小脑），通过 ROS 局域网进行高频通讯。其核心协同流程如下：
...
2. **TCP 补偿：** 针对 4 Pro 独有的夹爪偏心（3cm误差），彻底摒弃了官方巨大的 `temp_x/y` 粗调偏移，注入了毫米级的 `TCP_OFFSET_X` 和 `Y` 进行精密机械级修正。
...
# =================================================================
# 🔥 [核心机制 1]：硬编码物理补偿阵列
# 这里的参数是纯视觉降维带来的原罪，必须由人工测量后强行填补。
# =================================================================
COMPENSATE_X = 0.06         # 视觉测的是表面，夹爪需往前深探6cm包住瓶心
COMPENSATE_Y_LEFT = 0.03    # 抵消左手齿轮翻转后的 3cm 偏心距
COMPENSATE_Y_RIGHT = 0.14   # 抵消右手齿轮偏心距(3cm) + 抵消相机的右眼安装视差(约11cm综合误差)
COMPENSATE_Z = 0.12         # 把 0.25m 桌面高度强行拔高 12cm，瞄准瓶腰

CLAW_ROLL_RIGHT = 1.5708    # 右夹爪向右平放 (90度)
CLAW_ROLL_LEFT = -1.5708    # 左夹爪向左平放 (-90度)
# =================================================================
...
def get_horizontal_claw_quat(target_x, target_y, is_left_arm):
    ...
    # 2. 生成一个只绕着末端Z轴旋转 90 度的局部矩阵
    roll_angle = CLAW_ROLL_LEFT if is_left_arm else CLAW_ROLL_RIGHT
    cr, sr = math.cos(roll_angle), math.sin(roll_angle)
    R_local_z = np.array([[cr, -sr, 0], [sr, cr, 0], [0, 0, 1]])
    
    # 3. 矩阵右乘融合！彻底避免欧拉角Pitch为-90度时叠加Roll引发的奇异点崩溃
    return rotation_matrix_to_quaternion(R_base @ R_local_z)

def call_leju_claw(pos, vel, effort):
    try:
...
def main():
    ...
    # 🔥 [物理对抗补丁]：强行下发力矩锁住头部 Yaw=0.0，减少晃动误差
    head_pub.publish(robotHeadMotionData(joint_data=[0.0, 20.0]))
    time.sleep(1.5)
    ...
    # 🔥 [核心机制 3]：10帧数组中值滤波，彻底抹平 YOLO 框的单帧跳动和红外飞点
    while len(x_hist) < 10 and not rospy.is_shutdown():
        try:
            msg = rospy.wait_for_message('/vla/yolo_target', PointStamped, timeout=0.2)
            if 0.25 <= msg.point.x <= 0.60:  # 设置安全过滤门槛
    ...
    # 🔥 [核心机制 4]：算预瞄点 (防碰倒水瓶)。从终点倒推一条 12cm 的直线。
    ik_req.hand_poses.left_pose.joint_angles = list(q_grasp[:7])
    ...
    print(f"✊ 执行夹爪横向满力矩闭合...")
    if is_left_arm: call_leju_claw([90.0, 10.0], [50, 50], [1.5, 1.5])
    else: call_leju_claw([10.0, 90.0], [50, 50], [1.5, 1.5]) 
    time.sleep(2.0) 

    print(f"⬆️ 夺得水瓶，极高空垂直抬升...")
    execute_pose(arm_pub, q_lift, 2.0)

    print(f"⬅️ 保持高空，向后深度安全撤回 (避开正下方的书本)...")
    execute_pose(arm_pub, q_retract, 2.0)

    # 🔥 [核心机制 6]：放弃坐标系！跨界利用纯关节空间折叠，彻底解决贴胸时的逆解崩溃悖论
    print(f"🛡️ 关节空间直接折叠收手，规避 XYZ 坐标死区...")
    execute_pose(arm_pub, [math.radians(x) for x in ready_angles], 3.0)

    print(f"🤖 下放手臂，恢复自然下垂原状...")
    execute_pose(arm_pub, [math.radians(x) for x in init_angles], 3.0)
    
    # 🔥 [核心机制 7]：闭环修复，执行物理张开释放
    print("👐 任务完成，松开夹爪释放水瓶...")
    call_leju_claw([10.0, 10.0], [50, 50], [1.0, 1.0])
    time.sleep(2.0)
...
8. **服务调用的异步堵塞防御体系**：在实机调试中，硬件掉线或丢包是家常便饭。如果我们直接调用 `controlLejuClaw` 夹爪服务，一旦 Leju 驱动板出现哪怕 1 毫秒的通讯波动，整个 Python 主进程就会直接报出 `ServiceException` 导致程序闪退，正在半空中的机械臂也会瞬间失去指令流而断崖式坠落。因此，我们专门封装了带有 `try-except` 和 `rospy.wait_for_service(timeout=3.0)` 的调用外壳。这种防堵塞设计使得即使某一根手指偶尔没有收到信号，机械臂主干也依然能按照既定状态机平稳撤退，保住了上万元的硬件安全。
...
2. **强制剥夺并废除 TF2 算出的 Z 轴高度：** 无论 TF2 说这个瓶子在天上还是在地下，我们一概不听！我们将高度死死锁定在我们用尺子实测过的桌面绝对安全高度：`SAFE_LOCKED_Z = 0.37m`（0.25m桌面基准 + 0.12m目标补偿）。

**🔧 核心代码呈现：**

```python
    # 信任 TF2 传来的 X/Y，并施加夹爪的纯物理偏心补偿
    locked_x = raw_x + TCP_OFFSET_X
    if is_left_arm: locked_y = raw_y + TCP_OFFSET_Y_LEFT
    else: locked_y = raw_y + TCP_OFFSET_Y_RIGHT
    
    # 🔥 死死锚定在安全高度，将一切 Z 轴噪点拒之门外！
    locked_z = SAFE_LOCKED_Z  
    
    print(f"\n🎯 最终融合物理打击点: X={locked_x:.3f}, Y={locked_y:.3f}, Z={locked_z:.3f} (强锁)")
...
* **动作：** 将硬编码时期打磨出的毫米级微调参数（`TCP_OFFSET_X` = 0.005, `TCP_OFFSET_Y` = 0.05），无缝移植进 TF2 架构，用“人工物理补丁”补齐了 TF2 矩阵的最后一厘米。

**🔧 核心调参逻辑：**

```python
# 1. 深度补偿：既然 TF2 对准了瓶子表面，那我们就让 X 往后退一点，留出手指包络的空隙。
# 经过实测，我们把退回量敲定在了极其精确的 -0.005m (加上预留量，刚好夹紧瓶腰)。
TCP_OFFSET_X = 0.005  

# 2. 齿轮机械公差补偿：强行在 Y 轴拉回 5 厘米，抵消夹爪横置的机械臂甩动。
TCP_OFFSET_Y_LEFT = 0.05    
TCP_OFFSET_Y_RIGHT = 0.05  
...
# =================================================================
# 1. 抓取深度：TF2 给出的是表面绝对坐标。我们需要深入 0.5 厘米卡住瓶心
TCP_OFFSET_X = 0.005  

# 2. 抵消夹爪横置的机械偏心：
# 夹爪原本垂直向下，URDF建模只建到了法兰盘中心。旋转90度后，
# 原本上下的3cm齿轮厚度瞬间化作左右偏心距，TF2 算不到这个，必须人工拉回 3cm！
TCP_OFFSET_Y_LEFT = 0.03    
TCP_OFFSET_Y_RIGHT = 0.03  

# 3. 桌面安全高度锁 (绝对防死锁)：
# 🔥 TF2 会放大深度相机的边缘噪点。我们强行废弃 TF2 的 Z，定死在实测安全高度！
SAFE_LOCKED_Z = 0.37  

# 4. 全新 3D 避障通道参数：
LIFT_HEIGHT = 0.20          # 抓到后垂直向上拔高 20cm，越过下方黑书
AVOID_OUTWARD_Y = 0.10      # 向外侧(远离书本中心方向)强行平移 10cm，建立安全走廊
...
def main():
    ...
    while len(x_hist) < 10 and not rospy.is_shutdown():
        try:
            msg = rospy.wait_for_message('/vla/yolo_target', PointStamped, timeout=0.2)
            # TF2 绝对坐标 X 通常在 0.30 到 0.65 之间，设定过滤门槛
    ...
    # 🔥 核心融合：信任 TF2 的同时，用人工 TCP 物理偏移量对抗 URDF 未建模的机械公差
    locked_x = raw_x + TCP_OFFSET_X
    if is_left_arm: locked_y = raw_y + TCP_OFFSET_Y_LEFT
    else: locked_y = raw_y + TCP_OFFSET_Y_RIGHT
    
    # 强行抛弃会引发“天花板”悖论的视觉 Z 轴
    locked_z = SAFE_LOCKED_Z  
    ...
    ik_req.use_custom_ik_param = True; ik_req.joint_angles_as_q0 = True
    ik_param = ikSolveParam()
    # 设定极其严格的求解容差 (1e-3)，逼迫矩阵走完美直角，绝不允许抄近道
    ...
    q_high_safe = q_lift
    # 避障逻辑：尝试向左/向右侧移 10cm 的同时，向胸部方向后退 15~0cm。遇到死锁立刻降级！
    for fallback_dist in [0.15, 0.12, 0.09, 0.06, 0.0]:
        ratio = (dist - fallback_dist) / dist if dist != 0 else 1.0
        safe_x = robot_zero_x + (locked_x - robot_zero_x) * ratio
        
        # 灵魂避障：将手臂向身体外侧强行掰开，躲避正下方的书堆
        safe_y = locked_y + AVOID_OUTWARD_Y if is_left_arm else locked_y - AVOID_OUTWARD_Y
    ...
    # 核心动作：硬编码时期的纯关节折叠数组，作为我们最强的保底避障
    ready_angles = [40, 20, 0, -120, 0, 0, -20, 20, 0, 0, -30, 0, 0, 0] if is_left_arm else [20, 0, 0, -30, 0, 0, 0, 40, -20, 0, -120, 0, 0, -20]
...
9. **服务调用的异步堵塞防御体系**：在实机调试中，硬件掉线或丢包是家常便饭。如果我们直接调用 `controlLejuClaw` 夹爪服务，一旦 Leju 驱动板出现哪怕 1 毫秒的通讯波动，整个 Python 主进程就会直接报出 `ServiceException` 导致程序闪退，正在半空中的机械臂也会瞬间失去指令流而断崖式坠落。因此，我们专门封装了带有 `try-except` 和 `rospy.wait_for_service(timeout=3.0)` 的调用外壳。这种防堵塞设计使得即使某一根手指偶尔没有收到信号，机械臂主干也依然能按照既定状态机平稳撤退，保住了上万元的硬件安全。
...
* **进化方向：** 彻底打破“立正抓取”的限制。接入 ROS 的 Navigation 2 (Nav2) 与 SLAM 建图算法。当 YOLO 识别到远处的瓶子时，系统不再直接下发手臂指令，而是先解算出目标点相对于地图的全局坐标。随后，底盘系统接管控制权，驱动 Kuavo 走到目标桌前、调整好站立身位，最后再交由我们现在的 TF2 手臂系统完成抓取。
* **技术核心：** 眼在手外（Eye-to-hand）与眼在手上（Eye-in-hand）的标定转换，以及全身动态质心（CoM）在行走与伸臂时的实时平衡控制。

### 🚀 路线二：告别硬编码——纯动态轨迹避障 (Dynamic Obstacle Avoidance)

* **当前痛点：** 我们在 `auto_grasp_TF2.py` 中写死的 `LIFT_HEIGHT = 0.20`（拔高 20cm）和横向平移 10cm，虽然完美避开了正下方的书本，但这是一种极其死板的“L 型硬编码走廊”。如果障碍物不是在正下方，而是在水瓶的左边、右边或者上方，手臂依然会直直地撞上去。
