...
**适用硬件环境**: Lenovo Legion R9000P 2021 (AMD Ryzen 7 5800H + RTX 3070 Laptop 8GB VRAM + 16GB RAM)
**系统环境**: Ubuntu 20.04 LTS / 双显卡架构

---

## 0. 第零步：Docker 镜像提取 (高阶环境转移)

为了规避 Isaac Sim 在宿主机上繁杂的依赖安装 and 网络下载失败问题，我们采用了“借壳生蛋”的策略：先拉取官方/配置好的 Docker 镜像，然后将里面的核心引擎直接拷贝到宿主机。

### 0.1 启动并挂载容器

```bash
# 假设你已经拉取或载入了包含 Isaac Sim 4.2.0 的 Docker 镜像
...
由于 R9000P 2021 是 AMD 核显 + NVIDIA 独显的组合，在使用纯 Python 启动环境时，极易遭遇连环坑：
...
在 16GB 系统内存 + 8GB RTX 3070 的限制下，机器人数量 (`num_envs`) 和录像功能 (`--video`) 会对系统产生致命影响。官方推荐使用 `--headless`（无界面模式）进行训练。
...
> **架构师经验总结**：如果你的 `time_out` 一直在底下趴着，`base_contact` 居高不下，说明模型根本没学会走路（可能陷入了局部最优），不要浪费时间，直接停掉调整 Reward 权重或检查 Observation 设置重新跑。